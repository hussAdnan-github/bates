...
//                 currentIndex === index ? "border-[var(--primary_color)] scale-110" : "border-transparent opacity-70"
//               }`}
//             >
...
//             <div 
//               key={index} 
//               className={`h-1 rounded-full transition-all ${currentIndex === index ? "w-4 bg-[var(--primary_color)]" : "w-1 bg-gray-300"}`} 
//             />
//           ))}
//         </div>
        ...
        <div className="absolute right-2 md:right-3 top-1/2 -translate-y-1/2 z-20 flex flex-col gap-1.5 md:gap-2 bg-white/40 backdrop-blur-md p-1 md:p-1.5 rounded-full border border-white/50 transition-all duration-500
          ...
          {allImages.slice(0, 4).map((imgSrc, index) => (
            <button
              ...
              className={`relative w-6 h-6 md:w-7 md:h-7 rounded-full overflow-hidden transition-all duration-300 border-2 ${
                currentIndex === index 
